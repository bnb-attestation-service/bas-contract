...
async function initSchema(_registrySchema:string) {
    ...
    for (const {schema} of SCHEMAS) {
        const resp = await registrySchema.connect(signer).register(schema,ZERO_ADDRESS,true)
        console.log(`Registered schema ${schema} with UID ${getSchemaUID(schema, ZERO_ADDRESS, true)} in tx ${resp.hash}`);
    }
}
